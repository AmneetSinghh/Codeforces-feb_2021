...
cin>>n;
int a[n+1];
map<int,set<int>> mp;// positions is asceneing order;
F(i,n)cin>>a[i],mp[a[i]].insert(i);
vector<ipair> p;
for(auto c:mp)
{
 //    cout<<"-> "<<c.first<<"\n";
	// for(auto sd:c.second)cout<<sd<<" ";
	// 	tr;
    if(c.second.size()%2==1)
    {
    	...
    	for(auto h:c.second)
    	{
    		if(count%2==1)mp[c.first*2].insert(h);
    		++count;
    		pre=h;
    	}
    	p.pb({pre,c.first});

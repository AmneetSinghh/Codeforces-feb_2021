...
F(i,n){
	...
	for(int j=i-1;j>=0;j--)
	{
		if(a[i]-a[j]>=2*r-2)
		{
			dp[i]=max(mx[j]+1,dp[i]);
			break;
		}
		else if(a[i]-a[j]>=abs(b[i]-b[j])+abs(c[i]-c[j]))
			if(dp[j] || j==0)dp[i]=max(dp[i],dp[j]+1);
	}
...
//    for(int j=i+1;j<=n;j++)
//    {
//    	if(i==0  && a[i]+abs(b[j]-b[i])+abs(c[j]-c[i])<=a[j])dis[j]=max(dis[j],dis[i]+1);
//    	else if(dis[j] && a[i]+abs(b[j]-b[i])+abs(c[j]-c[i])<=a[j])dis[j]=max(dis[j],dis[i]+1);
//    }
// }

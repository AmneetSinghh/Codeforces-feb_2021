...
void dfs(int x,int current,int next)
{
    int i,sz=v[x].size();
    used[x]=1;
    if(goal[x]!=(init[x]^current))
    {
        o.push_back(x);
        current^=1;// bit is fliped it goes to the next state bro
    }
    for(i=0;i<sz;i++)
        if(used[v[x][i]]==0)dfs(v[x][i],next,current);
}

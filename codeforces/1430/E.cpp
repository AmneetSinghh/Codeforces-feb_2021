...
void Not_Stable() {
	...
	string s;cin>>s;
	string t=s;
	reverse(all(t));// resultant;
	set<int> store[100001] ;
	for(int i=0;i<s.size();i++)store[s[i]-'0'].insert(i+1);
   int ans=0;
    for(int i=0;i<t.size();i++)
    {
      auto pos=store[t[i]-'0'].begin();// position
      update(1,n,1,*pos,1);
      auto val=*pos+query(1,n,1,*pos+1,n);
      ans+=(val-(i+1));
      store[t[i]-'0'].erase(pos);
    }
